...
class AERP_Acc_Deposit_Table extends AERP_Frontend_Table
{
    public function __construct()
    {
        parent::__construct([
            'table_name' => $GLOBALS['wpdb']->prefix . 'aerp_acc_deposits',
            'columns' => [
                'code' => 'Mã',
                'deposit_date' => 'Ngày nộp',
            ...
            'sortable_columns' => ['code','deposit_date','total_amount','status','created_at'],
            'searchable_columns' => ['code','note'],
            'primary_key' => 'id',
            'per_page' => 10,
            'actions' => [],
            'bulk_actions' => ['delete'],
            'base_url' => home_url('/aerp-acc-deposits'),
            'delete_item_callback' => ['AERP_Acc_Deposit_Manager', 'delete_by_id'],
            'nonce_action_prefix' => 'delete_acc_deposit_',
            'message_transient_key' => 'aerp_acc_deposit_message',
            'hidden_columns_option_key' => 'aerp_acc_deposit_table_hidden_columns',
            'ajax_action' => 'aerp_acc_deposit_filter_deposits',
            'table_wrapper' => '#aerp-acc-deposit-table-wrapper',
        ]);
    }
    ...
    protected function get_extra_filters()
    {
        ...
        if (!empty($this->filters['receipt_id'])) {
            $filters[] = "receipt_id = %d";
            $params[] = (int)$this->filters['receipt_id'];
        }
    ...
    protected function column_receipt_id($item)
    {
        global $wpdb;
        $code = $wpdb->get_var($wpdb->prepare(
            "SELECT code FROM {$wpdb->prefix}aerp_acc_receipts WHERE id = %d",
            $item->receipt_id
        ));
        return esc_html($code ?: ('#' . intval($item->receipt_id)));
    }
    ...
    protected function column_status($item)
    {
        ...
        $map = [
            ...
            'cancelled' => '<span class="badge bg-dark">Hủy</span>',
        ];
    ...
    protected function column_order_row($item)
    {
        ...
        $lines = $wpdb->get_results($wpdb->prepare(
            "SELECT order_id, note FROM {$wpdb->prefix}aerp_acc_deposit_lines WHERE deposit_id = %d ORDER BY id ASC",
            $item->id
        ));
